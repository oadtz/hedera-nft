...
import { checkBalance, IAccount, transferNFT } from "../libs/hashgraph";
...
const UserCard: React.FunctionComponent<IUserCardProps> = ({
  ...
  const handleTransferNFT = async () => {
    await transferNFT(tokenId, account);
  ...
  return (
    <div className="w-2/3 bg-white shadow-lg rounded-lg overflow-hidden my-4 border border-gray-300">
      <img
        ...
        alt="avatar"
      />
      <div className="flex items-center px-3 py-3 bg-gray-900 hover:bg-gray-600">
        <a
          role="button"
          className="mx-3 text-white font-semibold text-lg"
          onClick={handleTransferNFT}
        >
          <img src="/icons8-money-96.png" className="w-8 h-8 inline mr-1" />
          Transfer NFT to this account
        </a>
      </div>
      <div className="py-4 px-6">
        ...
        <div className="flex items-center mt-4 text-gray-700">
          ...
          <h1 className="px-2 text-sm">
            NFT Count:{" "}
            {accountBalance?.tokens?._map.get(tokenId)?.toString() || 0}
          </h1>
        </div>
        <div className="flex items-center mt-4 text-gray-700">
          <svg className="h-6 w-6 fill-current" viewBox="0 0 512 512">
        ...
        <div className="flex items-center mt-4 text-gray-700">
          ...
          <h1 className="px-2 text-sm">Link to dragonglass</h1>
        </div>
      </div>
    </div>
  );
